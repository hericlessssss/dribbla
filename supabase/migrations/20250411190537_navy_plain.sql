/*
  # Fix GROUP BY clause in generate_group_stage_matches function

  1. Changes
    - Modify the generate_group_stage_matches function to properly handle teams.name in GROUP BY clause
    - Improve query structure to avoid GROUP BY issues
    - Add proper error handling for invalid parameters

  2. Technical Details
    - Use CTEs to better organize the query and avoid GROUP BY issues
    - Add validation for input parameters
    - Ensure proper team distribution across groups
*/
...
BEGIN
  ...
  FOR i IN 1..p_number_of_groups LOOP
    ...
    SELECT 
      ...
      tp.home_team_id,
      tp.away_team_id,
      (SELECT venue FROM teams WHERE id = tp.home_team_id),
    ...
    SELECT 
      ...
      tp.away_team_id,
      tp.home_team_id,
      (SELECT venue FROM teams WHERE id = tp.away_team_id),
